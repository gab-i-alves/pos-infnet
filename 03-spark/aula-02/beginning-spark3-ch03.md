...
**1.** Qual foi a abstração de programação central inicial do Spark, em que ano ela chegou, e em qual versão do Spark as Structured APIs foram introduzidas? *(Chapter intro)*
...
**2.** Quais duas coisas as Structured APIs exigem para que o Spark consiga otimizar? *(Chapter intro)*
...
**3.** Quais são as duas partes principais do módulo Spark SQL? *(Chapter intro)*
...
**4.** O que inspirou o conceito de DataFrame, e qual é a diferença principal? *(Chapter intro)*
...
**5.** Qual conceito diferencia dado estruturado de dado não estruturado, e como o capítulo o define? *(Chapter intro)*
...
**6.** Quais formatos de texto e quais formatos binários o capítulo nomeia como comuns para dado estruturado, e qual consequência não antecipada a versatilidade de leitura e escrita cria? *(Chapter intro)*
...
**7.** Qual novo grupo de usuários a capacidade de SQL traz para o Spark, segundo o capítulo? *(Chapter intro)*

R: Os business analysts, que já conhecem SQL porque é uma das ferramentas principais do dia a dia deles.

**8.** O que a Figure 3-1 mostra? Nomeie cada caixa e como elas se empilham. *(Figure 3-1)*
...
**9.** Dê a definição de RDD em uma frase que o capítulo apresenta, e liste as cinco características dele. Quais duas são opcionais? *(Understanding RDD)*
...
**10.** Quais três dessas cinco formam a lineage information, e para quais dois propósitos o Spark a usa? *(Understanding RDD)*
...
**11.** O que cada uma das três peças principais contribui? Quem fornece a função de computação e para onde ela é enviada? *(Understanding RDD)*
...
**12.** Qual é a desvantagem da flexibilidade do RDD, e quais três otimizações o capítulo nomeia como impossíveis por causa dela? *(Understanding RDD)*
...
**13.** Dê a definição de DataFrame do capítulo. Qual objeto representa cada linha? *(Introduction to the DataFrame API)*
...
**14.** Em quais dois tipos as APIs de DataFrame são classificadas, qual é a semântica de avaliação de cada um, e em quais quatro linguagens a API está disponível? *(Introduction to the DataFrame API)*
...
**15.** Qual é a única coisa comum a toda forma de criar um DataFrame? *(Creating a DataFrame)*

R: Fornecer um schema, de forma implícita ou explícita.

**16.** Qual função implícita converte um RDD em um DataFrame com nomes de coluna dados, e de onde vêm os tipos das colunas? *(Creating a DataFrame from RDD; Listing 3-1)*

R: A função implícita `toDF`, chamada como `rdd.toDF("key","value")`. Os tipos das colunas são inferidos a partir dos valores do RDD.

**17.** O que `printSchema` e `show` fazem, e quantas linhas o `show` exibe por padrão? *(Listings 3-2, 3-3)*
...
**18.** Qual função constrói um DataFrame a partir de um RDD de objetos `Row` mais um schema, e quais três informações cada `StructField` carrega? *(Listing 3-4)*
...
**19.** Qual flexibilidade o capítulo atribui a schemas criados programaticamente? *(after Listing 3-5)*

R: Que a aplicação Spark pode ajustar o schema com base em alguma configuração externa.

**20.** Pela Table 3-1, dê o tipo Scala de `DecimalType`, `BinaryType`, `TimestampType`, `DateType`, `ArrayType`, `MapType` e `StructType`. *(Table 3-1)*
...
**21.** O que o `spark.range` produz por padrão, e o que significam os três argumentos da forma mais longa? Qual limitação o capítulo aponta? *(Creating a DataFrame from a Range of Numbers)*
...
**22.** Qual é a opção que o capítulo dá para criar um DataFrame de várias colunas sem ler um arquivo? *(Listing 3-7)*
...
**23.** Quais são as duas classes principais para ler e escrever dados, e por quais variáveis elas são alcançadas? *(Creating a DataFrame from Data Sources; Writing Data Out to Storage Systems)*
...
**24.** Escreva o padrão comum de interação com o `DataFrameReader`, e diga quais das três peças da Table 3-2 são opcionais. *(Listing 3-9; Table 3-2)*
...
**25.** Liste as seis built-in data sources do Spark da Table 3-3 com o formato de dado de cada uma, e diga qual delas é a padrão. *(Table 3-3)*
...
**26.** Quais short names o `format()` aceita, e o que uma data source customizada exige? *(Listing 3-10)*
...
**27.** Quando o Spark lê um arquivo de texto, o que vira uma linha, como se chama a coluna resultante, e qual site o capítulo recomenda para livros gratuitos em texto puro? *(Creating a DataFrame by Reading Text Files; Listing 3-11)*
...
**28.** Liste as quatro opções de CSV da Table 3-4 com seus valores e seus defaults. *(Table 3-4)*
...
**29.** O que acontece se o `inferSchema` é false e nenhum schema é fornecido? Onde o capítulo diz que fica a lista completa de opções? *(Creating a DataFrame by Reading CSV Files)*
...
**30.** Como se lê um arquivo separado por tab com o formato CSV? *(Listing 3-13)*
...
**31.** Liste as três opções de JSON da Table 3-5 com seus defaults, e explique por que carregar um arquivo JSON muito grande é caro. *(Table 3-5; Creating a DataFrame by Reading JSON Files)*
...
**32.** Qual é a única desvantagem do JSON segundo o capítulo, e qual é a única coisa que você precisa informar ao Spark sobre um arquivo JSON? *(Creating a DataFrame by Reading JSON Files)*
...
**33.** O que o Spark faz por padrão quando encontra um registro corrompido ou um erro de parsing, e como se manda ele falhar rápido? Qual erro aparece? *(Listing 3-15)*
...
**34.** Onde o Parquet foi criado, por que ele é popular, como o tamanho dele se compara com o do CSV no exemplo do capítulo, e qual otimização de leitura o Spark aplica? *(Creating a DataFrame by Reading Parquet Files)*
...
**35.** O que a sigla ORC significa, quem a criou e por quê? *(Creating a DataFrame by Reading ORC Files)*
...
**36.** De quais quatro informações uma data source JDBC precisa, o que tem de estar no classpath, e como o capítulo testa a conexão? *(Creating a DataFrame from JDBC)*
...
**37.** Quais três opções de JDBC a Table 3-6 descreve? *(Table 3-6)*
...
**38.** O que é predicate pushdown, quais duas data sources o capítulo nomeia como suportando isso, e onde está o exemplo? *(Creating a DataFrame from JDBC)*
...
**39.** O que é uma DSL, e qual é o domínio de aplicação neste caso? *(Working with Structured Operations)*
...
**40.** Nomeie as structured transformations da Table 3-7. *(Table 3-7)*
...
**41.** Quais são as três categorias de funcionalidade da classe `Column`, e quando uma string não basta? *(Working with Columns)*
...
**42.** Liste as cinco formas de se referir a uma coluna na Table 3-8. Quais são exclusivas de Scala, e o que o autor recomenda? *(Table 3-8; Listing 3-21)*
...
**43.** Qual é o termo mais técnico para o `select`, e em que o `selectExpr` difere? *(Table 3-7; select; selectExpr)*
...
**44.** Quais operadores o `filter` usa para igualdade e para desigualdade, e quais são as duas formas de combinar condições? *(filter, where; Listing 3-26)*
...
**45.** Qual é a diferença entre `distinct` e `dropDuplicates`, e existe diferença de performance? *(distinct, dropDuplicates)*
...
**46.** Qual é a ordem de ordenação padrão, como se inverte ela, e o que o `limit` faz? *(sort, orderBy; limit)*
...
**47.** O que o `union` exige dos dois DataFrames, e o que o `withColumn` faz quando o nome da coluna já existe? *(union; withColumn)*
...
**48.** Em quais duas situações o `withColumnRenamed` é útil, e o que acontece quando a coluna informada não existe? E quanto ao `drop`? *(withColumnRenamed; drop)*
...
**49.** Quais são os três parâmetros do `sample`, e o que o `randomSplit` devolve? *(sample; randomSplit)*
...
**50.** Qual classe cuida de dado faltante, como ela é alcançada, e quais são as três formas comuns de lidar com o problema? *(Working with Missing or Bad Data)*
...
**51.** Qual é a diferença entre `na.drop("any")`, `na.drop("all")` e `na.drop(Array("actor_name"))`? *(Listing 3-36)*
...
**52.** Liste as structured actions da Table 3-9 e o que cada uma faz. *(Table 3-9)*
...
**53.** Quais estatísticas o `describe` calcula, e o que o exemplo do capítulo mostra? *(describe; Listing 3-37)*
...
**54.** Em qual versão do Spark as APIs de DataFrame e Dataset foram unificadas, por quê, e o que é um DataFrame do ponto de vista do código depois disso? *(Introduction to Datasets)*
...
**55.** Dê os sabores de Dataset por linguagem na Table 3-10, e explique por que Python e R só ganham um deles. *(Table 3-10)*
...
**56.** Quais são as duas propriedades únicas de um Dataset, para que servem os encoders, e quais são as limitações? *(Introduction to Datasets)*
...
**57.** Qual diretriz o capítulo dá para escolher entre Dataset e DataFrame? *(Introduction to Datasets; Working with Datasets)*
...
**58.** O que é uma case class do Scala, segundo a Note? *(Note, Introduction to Datasets)*
...
**59.** Quais são as três formas de criar um Dataset, qual é a mais popular, e o que o Spark valida? *(Creating Datasets; Listing 3-38)*
...
**60.** O que a Listing 3-39 prova sobre type safety? Dê os dois erros que ela dispara. *(Working with Datasets; Listing 3-39)*
...
**61.** Para qual caso de uso o SQL no Spark é desenhado, e para qual não? Qual revisão de SQL o Spark implementa, e qual benchmark decorre disso? *(Using SQL in Spark SQL)*
...
**62.** O que a Note diz que SQL é, e o que o torna diferente de Scala ou Python? *(Note, Using SQL in Spark SQL)*
...
**63.** Quais são as três opções para rodar SQL no Spark, quais duas integram o Hive, e qual delas o capítulo cobre? *(Running SQL in Spark)*
...
**64.** O que você precisa fazer antes de emitir SQL contra um DataFrame, e quais são os dois níveis de escopo? *(Running SQL in Spark)*
...
**65.** Quais funções registram os dois tipos de view, e o que `spark.catalog.listTables` e `spark.catalog.listColumns` devolvem? *(Listing 3-40)*
...
**66.** Qual função emite uma query SQL, o que ela devolve, e de qual prefixo uma global view precisa? *(Listing 3-41)*
...
**67.** Como se emite uma query SQL direto contra um arquivo de dados, sem registrar uma view? *(Listing 3-42)*

R: Nomeando o formato e o caminho em crases dentro do FROM, como em `spark.sql("SELECT * FROM parquet.`<path>/movies.parquet`")`.

**68.** Escreva o padrão comum de interação com o `DataFrameWriter`, e diga qual é a entrada do `save`. *(Writing Data Out to Storage Systems; Listing 3-44)*
...
**69.** Liste os save modes da Table 3-11 e diga qual é o padrão. *(Table 3-11)*
...
**70.** O que determina o número de arquivos de saída, como se descobre esse número, e como se força um arquivo único? *(Listings 3-46, 3-47)*
...
**71.** De onde vem a ideia de partitioning e bucketing, qual é a regra prática, e com o que se parecem os nomes de diretório resultantes? *(Writing Data Out to Storage Systems; Listing 3-48)*
...
**72.** Preencha a Table 3-12 para SQL, DataFrame e Dataset. *(Table 3-12)*
...
**73.** Quais APIs de persistência existem na classe DataFrame, e por que um DataFrame cacheado ocupa menos espaço que um RDD cacheado sobre o mesmo arquivo? *(DataFrame Persistence)*
...
**74.** Como a Listing 3-49 faz cache de um DataFrame com um nome legível por humanos, e o que força a persistência a acontecer? *(Listing 3-49)*
...
**75.** O que exatamente a Figure 3-2 mostra? Dê as colunas e a linha. *(Figure 3-2)*
...
**76.** Quais arquivos os exercícios usam, qual é o delimitador, e quais são as quatro tarefas? *(Spark SQL Exercises)*
...
**1.** Explique por que a flexibilidade do RDD bloqueia otimização, usando predicate pushdown como exemplo. *(Understanding RDD)*
...
**2.** Explique o que o schema dá ao Spark, além de documentar o dado. *(Chapter intro; DataFrame Persistence)*
...
**3.** Explique o que "lazily evaluated" significa na prática para uma cadeia como `movies.filter(...).select(...).show(5)`. *(Introduction to the DataFrame API; Working with Structured Actions)*
...
**4.** O capítulo insiste que um DataFrame é imutável. Explique o que o `union` e o `withColumn` de fato fazem, dado isso. *(Table 3-7; union; withColumn)*
...
**5.** Explique quando você precisa passar uma instância de `Column` em vez de uma string, usando `kvDF.select('key, 'key > 1)` como caso. *(Working with Columns; Listing 3-21)*
...
**6.** Explique o trade-off entre `inferSchema=true` e um schema escrito à mão, usando a coluna `year` do CSV da Listing 3-12. *(Creating a DataFrame by Reading CSV Files)*
...
**7.** Explique por que o `samplingRatio` existe para JSON e do que você abre mão ao baixá-lo. *(Creating a DataFrame by Reading JSON Files)*
...
**8.** Explique por que o Parquet é o formato padrão, comparando-o com CSV e JSON nos eixos que o capítulo nomeia. *(Creating a DataFrame by Reading Parquet Files)*
...
**9.** Explique o que o capítulo quer dizer com "the metadata catalog", e como uma temporary view se relaciona com uma variável DataFrame. *(Running SQL in Spark; Listing 3-40)*
...
**10.** Explique a Table 3-12 linha por linha: por que o SQL não pega nada em tempo de compilação, por que o DataFrame pega um tipo de erro e não o outro, e por que o Dataset pega os dois. *(The Trio: DataFrame, Dataset, and SQL)*
...
**11.** Explique a relação entre as partitions de um DataFrame e os arquivos de saída, e por que o `coalesce(1)` é chamado de truque. *(Listings 3-46, 3-47)*
...
**12.** Explique por que o capítulo trata `select`, `selectExpr` e `spark.sql` como três formas de fazer a mesma projeção, e o que de fato os distingue. *(selectExpr; Running SQL in Spark)*
...
**1.** Você recebe logs de acesso de servidor como arquivo de texto com `|` como delimitador e sem linha de cabeçalho. Escreva a chamada de leitura com schema explícito, e diga quais opções da Table 3-4 você precisa e quais você não pode usar.
...
**2.** Você tem leituras de sensor em JSON em que cada objeto atravessa várias linhas, e alguns registros estão quebrados. Mostre a chamada de leitura e os dois comportamentos entre os quais você pode escolher.
...
**3.** Você precisa dos pedidos de um mês de um RDBMS de e-commerce. Escreva a leitura JDBC e diga exatamente onde o filtro roda.
...
**4.** Pegue um dataset aberto de viagens de bicicleta de uma cidade. Produza viagens por estação, ordenadas de forma descendente, de três formas: operações de DataFrame, `selectExpr` e SQL.
...
**5.** Um parceiro te entrega um Parquet com 120 colunas e nomes crípticos. Você precisa de seis delas, com nomes legíveis. Quais transformations você usa e por que não as outras?
...
**6.** Divida um dataset rotulado em treino, validação e teste em 70/15/15, e prove que as contagens fecham.
...
**7.** Escreva um DataFrame de sensores para a saída de forma que ler um único dia seja barato. Mostre a chamada, descreva o diretório de saída, e diga o que quebra se você particionar por id de sensor em vez disso.
...
**8.** Converta um DataFrame de catálogo de filmes em um Dataset com uma case class, e depois mostre uma operação que falha em tempo de compilação e a gêmea dela em DataFrame que falha em runtime.
...
**9.** Você tem um DataFrame de catálogo em que algumas linhas não têm identificador nenhum. Descarte só as linhas que estão inteiramente vazias, e depois descarte as linhas sem identificador. Escreva as chamadas e diga qual é a diferença.
...
**10.** Escreva um relatório como um arquivo CSV único com `;` como delimitador, sobrescrevendo o que estiver lá. Escreva a chamada e nomeie as três coisas que ainda podem dar errado.
...
**1.** **O alias circular.** A Table 3-9 diz "first is an alias for first. take(n) is an alias for first(n)." Reconstrua o grafo de aliases que a tabela pretendia descrever, e explique como um defeito desses sobrevive.
...
**2.** **A tabela que discorda da própria legenda.** A Table 3-12 tem a legenda "Syntax and Analysis Errors Spectrum", mas a primeira linha é rotulada "System Errors". Decida qual rótulo está correto, e defina as duas linhas.
...
**3.** **A referência cruzada apontando para o lugar errado.** A seção DataFrame Persistence diz que todas as opções de storage "described in Table 3-5" se aplicam a persistir um DataFrame. Abra a Table 3-5. Para onde a referência deveria apontar?
...
**4.** **A união dupla.** A Listing 3-30 chama `union` duas vezes mas mostra cinco linhas. Trace a aritmética e dê a saída que o código de fato produziria.
...
**5.** **A figura que contradiz o sumário.** A Figure 3-1 desenha o módulo Spark SQL. O próprio Summary do capítulo diz que a abstração de programação principal no Spark SQL é o Dataset. Reconcilie.
...
**6.** **A afirmação sobre o Parquet.** O capítulo afirma que o Parquet "stores each column's data in a separate file". Teste essa afirmação contra a própria Listing 3-16 do capítulo.
...
**7.** **Duas contagens do mesmo dataset.** A Listing 3-35 reporta `movies.count` como 31393. A Listing 3-37 reporta `describe("produced_year")` com count 31392. Explique a diferença.
...
**8.** **O caminho que denuncia a edição anterior.** Todo caminho de dado do capítulo diz `chapter4/data/movies`, mas os exercícios dizem `chapter3/data/movies`. Ache a segunda evidência que data os listings, e diga o que aconteceu.
...
**9.** **A variável que não é impressa.** As Listings 3-13 e 3-14 criam `movies4` e `movies5`, e depois chamam `movies.printSchema`. Diga o que o leitor de fato vê, e se a demonstração ainda funciona.
...
**10.** **O listing cortado.** A Listing 3-1 mostra `Random.nextInt(100` e a linha de código é cortada na margem da página. A Listing 3-2 mostra valores como 237, 567 e 360. Deduza o argumento real e diga o que essa classe de defeito custa.
...
**11.** **`filler`.** Um cabeçalho de seção diz "filler(condition), where(condition)". Liste os outros defeitos da mesma classe neste capítulo, e diga o que eles têm em comum.
...
**12.** **A promessa do writer.** A Listing 3-44 mostra `partitionBy(...).bucketBy(...).sortBy(...)` e o texto diz que você aprende mais sobre isso "later in the chapter". Audite o que o capítulo entrega.
...
**13.** **Schema opcional?** A Table 3-2 lista o `schema` como opcional. A Table 3-4 diz que com `inferSchema` false e sem schema, toda coluna é string. Reconcilie as duas, e decida quando o schema é genuinamente opcional.
...
**14.** **Duas formas de somar uma coluna.** `withColumn` e `selectExpr` acrescentam uma coluna, e o capítulo diz que os dois cumprem "pretty much the same goal". Ache onde eles divergem.
...
**15.** **O que o capítulo adia.** Liste tudo o que este capítulo nomeia e adia. Julgue se o adiamento é honesto.
...
**16.** **A escolha do apóstrofo.** A Table 3-8 recomenda `'columnName` para uso diário, e todo exemplo do capítulo usa isso. Pese essa escolha.
...
**1.** **Versão atual e matriz de suporte.** O capítulo mira o Spark 3.1.1 com Scala 2.12. Ache a versão corrente, as versões suportadas de Java, Scala, Python e R, e se alguma arquitetura de deployment nova apareceu desde 2021.
...
**2.** **Table 3-1 hoje.** O capítulo lista quinze tipos do Spark. Ache quais foram acrescentados desde 2021.
...
**3.** **Table 3-3 hoje.** O capítulo lista seis built-in data sources. Ache o conjunto atual.
...
**4.** **Table 3-4 hoje.** Confira os defaults das quatro opções de CSV, e descubra se o `mode` pertence ao CSV também.
...
**5.** **Table 3-5 hoje, e o que o modo padrão realmente faz.** Confira os três defaults de JSON, e confira a afirmação do capítulo de que, num registro corrompido, o Spark "sets the value for all the columns in that row to be null".
...
**6.** **Formato padrão, save modes e o `bucketBy`.** Confirme que o Parquet ainda é o padrão, liste os save modes atuais, e confira se o `bucketBy` funciona com o `save`.
...
**7.** **ANSI SQL.** O capítulo diz que o Spark implementa um subconjunto do ANSI SQL:2003. Ache o que a documentação atual diz sobre ANSI compliance.
...
**8.** **O apóstrofo.** Todo exemplo do capítulo usa `'columnName`. Confira o status dos symbol literals do Scala.
...
**9.** **Storage levels e o nível padrão.** O capítulo aponta para a tabela errada quanto às opções de storage. Ache a lista real, e reconcilie-a com o que a Figure 3-2 mostra.
...
**10.** **A estrutura do Parquet.** O capítulo diz que cada coluna vai para um arquivo separado. Confira a especificação.
...
**11.** **Opções de JDBC hoje.** A Table 3-6 lista três opções. Ache o que existe hoje, especialmente para paralelismo.
...
**12.** **O catalog hoje.** A Listing 3-40 mostra o `listTables` devolvendo uma coluna `database`. Confira o formato atual.
...
**13.** **`describe` contra `summary`, `union` contra `unionByName`.** Ache os irmãos mais novos de duas operações que o capítulo ensina.
...
**14.** **Temporary views hoje.** Confira se o escopo de sessão e o global descritos no capítulo ainda valem.
